...
- September 23 update — OpenAI Codex (GPT-6), task “Astra audit Claude website changes”: Bridget now authorizes the current main-checkout checkpoint and push, limited to the current website and necessary files. Older versions, draft galleries, and superseded exports must remain local. Verify delivery before marking it pushed. Do not merge stale website code or include oversized raw audio/DAW sources; preserve those in the documented local backup. No domain cutover is authorized.
