...
**Synchronization verified:** `7b5a38c128a1d8de38e75dbe18f173e75cd1b5aa` pushed to `origin/main`; local/remote matched and working tree was clean. GitHub Pages run 33964659571 succeeded. The deployed Team page passed the 390px vertical-profile check with all five names visible, no overflow/broken images, and noindex retained. This documentation follow-up records that result.


## 2026-09-05 — Finish the assessed polish while preserving Claude's direction

**Request:** Bridget approved the assessment recommendations and specified that the most recent Claude work remains the main direction. This pass starts from `43efad2`, preserving all 15 Claude polish commits plus the newer vertical Team layout and handoff/cache reconciliation.

**Hero persistence and framing:** `safeConfig()` now retains the boolean `mobileFramed` flag. Previously a manual phone edit worked immediately but reverted to derived desktop framing after reload, even when the exported JSON contained the flag. The same four existing photographs now have explicit canonical phone crops: microphone 68/50/110, guitarist 0/75/170, sign 74/50/100, rack 50/50/100 (x/y/zoom percentages). These show the microphone, focus the guitar rather than the face, and retain the sign/rack subjects. Image paths, order, desktop profiles, other media slots, and the separate proposed reference export are unchanged. Phone framing is implemented for review, not recorded as an approved screenshot baseline.

**Palette and inquiry:** Shared champagne fill/hover tokens now drive the header, primary page buttons, and the previously amber phone-menu CTA. Inquiry placeholders change from `#8d8478` to `#6a6157` on white (3.68:1 to 6.07:1); focus outlines use the existing deep bronze `#7a5a33`. The contact card says “choose a time in the calendar” so it remains accurate beside the form on desktop and below the calendar on phones. The consultation panel remains parked.

**Services and caches:** Reduced only the desktop title's viewport scale so “Services” fits on one line at intermediate desktop widths. The six-state interaction and phone composition are retained. Shared header/theme/responsive/title-panel styles, hero script, shared inner-page loader, and Services stylesheet use fresh `20260905-2` keys; all nine inner pages reference the refreshed loader.

**Scope:** No affiliates redesign, new imagery, master-media upload, media deletion, inquiry send, booking, production indexing, or Wix/domain cutover. Missing tour/WAV references remain the documented migration work. The original Team layout and Claude's desktop homepage direction are preserved.

**Validation:** All ten content pages passed browser checks at 390px and 1440px: no horizontal page overflow, broken loaded images, or JavaScript exceptions; development noindex retained and refreshed shared styles loaded. All six Services selections worked and its title stayed on one line at both widths. Phone menu CTA matches the primary-button gradient; all five review heights fit their content. Phone hero crops match canonical data, and a manual crop survives browser reload, JSON export, and a fresh browser loading that exported canonical JSON. Desktop Services, phone Team/menu, and all four phone hero images were visually reviewed. JavaScript syntax and whitespace checks pass. Tracked-file validation reports only the three previously documented missing tour/WAV references. No inquiry or calendar submission was made.


## 2026-09-05 — Center the affiliates banner and remove the empty cell

**Request:** Bridget supplied a screenshot of the boxed affiliates grid and asked to continue fixing it. This implements the existing Claude handoff: five centered logos, three over two on phones, no box separators, and a banner arrangement on desktop.
...
## 2026-09-23 — Fifteen-second website audio samples
