...
Status: Active procedure for root website and exploratory design work
Applies to: the canonical root website and any explicitly active visual experiment
Does not authorize: public launch or labeling a design as approved without explicit approval
...
Record the page, section, component, viewport, and canonical file. Do not expand a localized request into unrelated restyling.
...
Keep or identify a screenshot of the affected surface before editing. Record the reference in `WEBSITE_CHANGE_LOG.md`. Do not put it in `APPROVED_VISUAL_BASELINES/` unless the requester explicitly approves it.
...
Add an entry to `WEBSITE_CHANGE_LOG.md` containing:
...
Approval of one section does not approve the whole page or authorize public launch.
